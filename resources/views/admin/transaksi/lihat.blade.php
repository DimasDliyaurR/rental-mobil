                            ...
                            <div class="row justify-content-between mt-4">
                                ...
                                <div class="col-md-4">
                                    <form action="/transaksi">
                                        @csrf
                                        <label for="search" class="me-2 align-middle" style="color: rgba(1, 1, 1, 0)">cari</label>
                                        <div class="input-group mb-3">
                                            <input type="text" class="form-control" placeholder="Cari nama penyewa, kendaraan, plat kendaraan..." value="{{ request('search') }}" name="search" id="search">
                                    ...
                                    @if ($data->count())
                                        @foreach ($data as $row)
                                            <tr>
                                                ...
                                                <td class="text-capitalize text-center">{{ $row->nama_merek . ' ' . $row->nama_brand }}</td>
                                                <td class="text-uppercase text-center">{{ $row->plat }}</td>

                                                <td class="text-center"><a href="{{ asset('/transaksi/detail_transaksi/' . $row->id) }}">Lihat
                                                        Detail</a></td>
                                                <td class="text-center">{{ date('d-m-Y', strtotime($row->tanggal_sewa)) . ' ' . date('h:i:s', strtotime($row->waktu_kembali)) }}
                                                </td>
                                                <td class="text-center">
                                                    <a href="{{ asset('transaksi/invoice/' . $row->id) }}" class="btn btn-success">
                                                        <i class="bi bi-receipt"></i>  Invoice
                                                    </a>
                                                </td>
                                                <td class="text-center">
                                                    <a href="{{ asset('/transaksi/update/' . $row->id) }}" class="btn btn-info me-2">
                                                        <i class="bi bi-pencil-square"></i>  Update
                                                    </a>
                                                    <a href="{{ asset('transaksi/delete/' . $row->id) }}" class="btn btn-danger">
                                        ...
                                        @endforeach
                                    @else
                                        <tr><td colspan="7" class="text-center">Transaksi tidak ada.</td></tr>
                                    @endif
                                </table>
                            </div>
